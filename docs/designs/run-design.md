...
- `npm test` / `npx tsx --test tests/run-server.test.mjs` (integración de servidor, 18 casos escritos): bloqueado en este sandbox por el binario nativo de esbuild (`@esbuild/win32-x64` presente, `@esbuild/linux-x64` ausente) — limitación ambiental ya documentada en sprints previos (ATK-RIM, MOVE-WITHDRAW), no relacionada con la corrección del código. `npm test` completo intentado: 20/70 pasan (los que no requieren transformación real), 50 fallan por la misma causa raíz de `tsx`. Pendiente de ejecución real en Windows.
- `npm run build`: `build:shared` y `build:server` compilan limpio (`tsc` puro, sin esbuild). `build:web` bloqueado por binario nativo de Rollup faltante para Linux en este sandbox — no relacionado con el código de Correr.
- `node scripts/e2e-websocket.mjs`: se intentó además arrancar `apps/server/dist/index.js` (compilado con `tsc` puro) directamente con `node`, evitando `tsx` — pero `@dnd-tactical/shared` resuelve en runtime a `src/index.ts` (`package.json: "main": "src/index.ts"` del workspace), que exige `tsx` igualmente. Mismo bloqueo de raíz, no un problema nuevo. `npm run test:ui` (Playwright): bloqueado porque su `webServer` depende de `vite dev` (mismo bloqueo de Rollup). Ambos pendientes de validación en Windows.
- Regresión: se ejecutó la suite completa de tests puros (`tests/*.test.mjs` sin dependencia de `tsx`) antes y después de los cambios de este sprint (vía `git stash`); los mismos 11 fallos preexistentes aparecen en ambos estados (idénticos, incluida su causa), confirmando que son ajenos a MOVE-RUN y no una regresión introducida aquí.

**Dictamen de esta sección**: implementación completa según el alcance acotado (§6), typecheck estricto verde, tests puros verdes, tests de servidor/E2E/UI escritos pero no ejecutables en este entorno — consistente con el patrón ya aceptado en sprints anteriores para este mismo sandbox. **DoD no completo**; cierre formal del Sprint 041 pendiente de la validación real en Windows (ver `walkthrough.md`).
