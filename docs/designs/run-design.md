...
### Dudas registradas (no resueltas por este documento, requieren decisión explícita)

- **D-1 (AdO por la acción en sí)**: la Tabla 8-2 declara dos veces en el corpus (`05_acciones.txt:143`, `08_ataques_de_oportunidad.txt:88`) que "Correr" provoca Ataque de Oportunidad como columna propia de la acción — a diferencia de "Retirarse del combate", que la misma tabla marca expresamente como "No". Esto es contraintuitivo (correr es solo movimiento; el AdO "por la acción en sí" no tiene un mecanismo obvio distinto del AdO por abandonar una casilla amenazada, que ya se dispara por movimiento normal). No se interpreta ni se descarta: se registra como hecho literal del corpus, repetido en dos archivos, y se deja para que el propietario confirme si se implementa tal cual (un AdO adicional garantizado al iniciar la acción, redundante con el de movimiento) o si se trata de un artefacto de la tabla oficial que no debe traducirse a una mecánica adicional.
- **D-2 (resistencia multi-asalto)**: el sistema de "rondas = Constitución, luego pruebas de Constitución con CD creciente, descanso de 10 asaltos" introduce estado persistente entre turnos que hoy no existe en ningún lugar del motor (todo el estado de economía vive en `TurnState`, que se reinicia cada turno). Se propone como **fuera de alcance de este sprint** (ver §6) — pendiente de aprobación explícita de esa acotación.
- **D-3 (pérdida de Destreza, no de Esquiva)**: el texto dice que se pierde *solo* el bonificador de Destreza a la CA, conservando el de Esquiva. La única supresión existente en el motor (`NO_DEX_TO_AC`, usada por Desprevenido) suprime **Destreza y Esquiva juntos** (`shouldApplyAcModifier`, `rules.ts:186`). Reutilizar ese trait tal cual sería una simplificación no-RAW (penalizaría también la Esquiva). Se registra como decisión abierta: reutilizar la supresión conjunta existente documentando la simplificación, o construir una supresión más granular específica de Destreza.
- **D-4 ("no ver hacia dónde vas")**: no existe todavía ningún modelo de visión/Cegado en el motor (deuda ya registrada en el Sprint MOVE-WITHDRAW: "exención pro-defensor ante invisibles y Cegado sin validar"). Esta restricción no puede implementarse de forma real sin ese modelo — se propone diferirla explícitamente hasta que exista.
