...
**Atomicidad**: handler síncrono único — validar todo (economía, ruta, exenciones) antes de mutar; commit de posición+economía+log en un solo paso, patrón `handleCharge`. **Interrupción a mitad de ruta**: posición final = última legal; economía consumida sin reembolso (idéntico al movimiento vigente). **Estados imposibles**: prevenidos por los pre-checks de entrada más la malla de gates ya existente; cero campos nuevos en `TurnState`.
