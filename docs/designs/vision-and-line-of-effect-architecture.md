...
### 1.3. Cover (`getAttackLineInterception`, Sprint 013)

Documentado en `docs/designs/cover-and-dynamic-reach-design.md`. Es una función pura que detecta **solo criaturas vivas** (`active`/`disabled`) interpuestas en el segmento atacante-objetivo mediante colinealidad exacta (producto cruzado) y pertenencia al segmento (producto punto). Produce `CoverAssessment { applies, acBonus, kind, blockerIds, blockedCellKeys }`. **Deliberadamente fuera de su alcance** (declarado en su propia Sección 10): "cobertura por muros, puertas, mobiliario o terreno del mapa", "concealment, miss chance, línea de visión e iluminación", "Cobertura Total y bloqueo efectivo de la acción". Es decir: **Cover de criatura y Cobertura Total por terreno sólido son, en el código y en el diseño, dos mecánicas explícitamente distintas que todavía no existen ambas** — solo la primera está implementada.

Verificación de código: en el modelo actual, `board.impassableCells` (celdas marcadas como intransitables para movimiento, ya declarado en `types.ts:29`) **no se consulta en `getAttackLineInterception` ni en `getCellsIntersectedByAoE`**. Un muro bloquea hoy el movimiento (`chargeResolver.ts`, `roomState.ts`, `viewModel.ts`) y el corte de esquina diagonal (`isCornerAnchorBlockedByTerrain`, Sprint 037), pero **no bloquea líneas de ataque, cobertura, ni áreas de efecto**. Este es el gap central que Line of Effect debe cerrar.

**Corrección de esta revisión**: `impassableCells` es hoy una noción de *intransitabilidad para movimiento*, no una noción declarada de *opacidad física*. El modelo actual asume, como aproximación de trabajo, que una celda intransitable también es sólida a efectos de propagación física — pero el propio catálogo de casos futuros (cristales, barrotes, rejas) demuestra que "intransitable" y "opaco a un efecto/línea" no son necesariamente el mismo predicado. Este documento reutiliza `impassableCells` como fuente de partida más cercana disponible, no como sinónimo definitivo de obstáculo sólido para Line of Effect; la auditoría normativa específica de superficies parcialmente transparentes queda para Sprint 052 o posterior (ver §8, pregunta 9, y §11).

`getAttackLineInterception` **no se generaliza** para cubrir este gap. Ver Corrección C en §3 y §6.
