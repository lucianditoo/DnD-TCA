...
- **Fase Actual:** Sprint 042.5 completado — baseline recuperado a 430/430, Sprint 042 (Cover) cerrado formalmente. Pendiente de confirmación de GitHub Actions tras el push de este cierre.
...
**Cierre (Sprint 042.5, 2026-07-18):** la duda quedó resuelta — los 9 fallos (7 reales) fueron investigados uno por uno, clasificados y corregidos por causa raíz (no un parche): 5 Environmental Hazards eran un bug real de producción (`cloneEffectInstances` no propagaba `targetCells`, ver DT-021), 1 Ray of Frost era un test desactualizado frente a una regla posterior correcta (`ATK-RANGED-INTO-MELEE`), y 1 Withdraw W22 era un regex mal escrito en el propio test. `npm test` queda **430/430**; typecheck, build, E2E WebSocket (87/87) y Playwright (5/5) también en verde real en esta máquina. **Sprint 042 (Cover) queda cerrado formalmente.** Próximo paso recomendado: confirmar que el próximo run de GitHub Actions (tras este push) también cierra en verde, y solo entonces evaluar el siguiente sprint funcional (Concealment o Conditions), verificando primero — como ya se hizo para Cover — qué infraestructura relevante podría estar ya parcialmente construida antes de diseñar desde cero.
