...
- **Fase Actual:** Sprint 053B.1, revisión arquitectónica de Vision — integración publicada aprobada; Darkvision respeta `darkvisionFeet` tanto en oscuridad como al suprimir la ocultación parcial de luz tenue.
- **Estado:** ✅ Vision/iluminación básica y targeting a ciegas · ✅ revisión arquitectónica 053B.1 · ✅ 544/544 unitarias · ✅ 100/100 WebSocket · ✅ Playwright 7/7.
- **Hitos Completados Recientes:**
  - `Sprint 053B/053B.1`: `Board.dimLightCells`/`darknessCells` + transporte por Snapshot; `IntrinsicPerception.darkvisionFeet`; `VisualPathAssessment`; `VisionAssessment` compuesto con `ConcealmentAssessment`; targeting a ciegas anti-metagaming por casilla. La revisión publicada añadió la regresión de Darkvision dentro/fuera de alcance bajo luz tenue. 19 tests en `tests/vision-core.test.mjs`, 15 en `tests/blind-targeting-server.test.mjs`.
...
### HISTÓRICO: Sprint 053B — Vision, iluminación básica y targeting a ciegas
  ...
  **Sprint 053 / 053A / 053A.1 / 053A.2 (diseño, sin código) + Sprint 053B (implementación)**
  ...
  - Tests: `tests/vision-core.test.mjs` (19 tras revisión 053B.1, unitarios de Vision + composición), `tests/blind-targeting-server.test.mjs` (15, targeting directo/por casilla + regresiones de Cover/LoE/Blinded).
  - **Validación real ejecutada (2026-07-24, 053B.1)**: `npm test` **544/544**, `npm run typecheck` 0 errores (3 workspaces), `npm run build` los 3 en verde, `node scripts/e2e-websocket.mjs` **100/100** aserciones exit 0, `npm run test:ui` **7/7** escenarios Playwright.
