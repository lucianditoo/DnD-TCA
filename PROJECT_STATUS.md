...
- **Fase Actual:** Sprint 053B, Vision/iluminación básica y targeting a ciegas — `VisualPathAssessment`/`getVisualPathAssessment` (geometría, extraído de `getLineOfEffect` sin duplicar el algoritmo), `VisionAssessment`/`getVisionAssessment` (luz/Darkvision, 5 reglas de precedencia), composición con `ConcealmentAssessment` (severidad máxima, nunca suma), y `resolve-attack` extendido con `target: {kind:"combatant"}|{kind:"square"}` para targeting a ciegas contra Ocultación Total (server-authoritative, sin filtrar si la casilla estaba vacía).
- **Estado:** ✅ Aplicar/remover condiciones desde la UI real · ✅ onStack delegado íntegramente a `EffectManager` · ✅ Legalidad de objetivo (Line of Effect) antes de tirada/mutación en `resolve-attack`, con geometría corregida · ✅ Vision/iluminación básica y targeting a ciegas (Sprint 053B) · ✅ 542/542 unitarias · ✅ 100/100 WebSocket · ✅ Playwright 7/7.
- **Hitos Completados Recientes:**
  - `Sprint 053B`: `Board.dimLightCells`/`darknessCells` + transporte por Snapshot; `IntrinsicPerception.darkvisionFeet` (Combatant/Snapshot/CreatureTemplate); `VisualPathAssessment` (geometría pura, siblings de `LineOfEffectAssessment`, comparte `lineOfEffectBlockingCells` provisionalmente por decisión de NDD); `VisionAssessment`/`VisionReason` (`clear`/`dim-light`/`darkness`/`blocked-visual-path`/`darkvision-out-of-range`) compuesto con `ConcealmentAssessment` (total > partial > none, miss chance nunca se suma); `resolve-attack` acepta `target` discriminado por casilla u objetivo directo, con targeting a ciegas anti-metagaming (casilla vacía consume intento/acción/munición sin revelar el motivo del fallo). Precedencia crítica descubierta: Cobertura Total (Line of Effect) se evalúa antes que Ocultación Total (Vision) porque ambas comparten la misma fuente de bloqueo provisional. 17 tests nuevos en `tests/vision-core.test.mjs`, 15 en `tests/blind-targeting-server.test.mjs`.
  - `Sprint 053/053A/053A.1/053A.2` (solo diseño): corrigió la confusión conceptual `LineOfEffectAssessment → VisionAssessment` en el NDD; diseñó la primera vertical funcional de Vision con iluminación básica (`docs/designs/vision-and-line-of-effect-architecture.md` §13); nombró el internal geometric layer `VisualPathAssessment` (sin redefinir el término normativo "Line of Sight"); retiró Low-Light Vision del alcance (infraestructura sin consumidor); diseñó el flujo completo de targeting a ciegas y anti-metagaming (Projection Layer, payload seguro).
...
### FASE ACTUAL: Sprint 053B — Vision, iluminación básica y targeting a ciegas

  **Sprint 053 / 053A / 053A.1 / 053A.2 (diseño, sin código) + Sprint 053B (implementación)**
  - Origen: auditoría Sprint 053 detectó que el NDD previo confundía `LineOfEffectAssessment` con una futura `VisionAssessment` (relación `LoE → Vision` incorrecta); corregido a una geometría común que se ramifica en Line of Effect y una capa geométrica interna independiente. Sprint 053A diseñó la primera vertical funcional de Vision con iluminación básica (sin implementar código); Sprint 053A.1 cerró 4 bloqueadores del diseño (nombre canónico `VisualPathAssessment` para la capa geométrica interna — nunca redefine el término normativo "Line of Sight" —, retiro de Low-Light Vision del alcance por falta de consumidor real, diseño completo del flujo de targeting por casilla, alcance parcial explícito de oscuridad); Sprint 053A.2 añadió el diseño de "Blind Targeting y Anti-Metagaming" (separación estado autoritativo/observable, payload seguro, Projection Layer). NDD final: `docs/designs/vision-and-line-of-effect-architecture.md` §13.
  - **`Board.dimLightCells`/`darknessCells`** (transportados por `CombatRulesSnapshot`, oscuridad domina sobre luz tenue en celdas superpuestas) + **`IntrinsicPerception.darkvisionFeet`** (Combatant/Snapshot/CreatureTemplate, deliberadamente sin `lowLightVision` — infraestructura sin consumidor real es exactamente el error que este proyecto ya evitó una vez con `impassableCells`/Cover).
  - **`VisualPathAssessment`/`getVisualPathAssessment`** (`rules.ts`): capa puramente geométrica ("¿hay una ruta visual clara considerando obstáculos opacos?"), hermana de `LineOfEffectAssessment` — no un alias. Extraída mecánicamente de `getLineOfEffect` en un helper privado compartido (`computeSupercoverPathAssessment`) reutilizando el recorrido "supercover" de Sprint 052B.1 sin duplicar el algoritmo; ambas funciones comparten provisionalmente `Board.lineOfEffectBlockingCells` como fuente de datos, autorizado explícitamente por el NDD.
  - **`VisionAssessment`/`getVisionAssessment`**: 5 reglas de precedencia (ruta bloqueada → oscuridad sin Darkvision → oscuridad fuera de alcance de Darkvision → luz tenue sin Darkvision → claro), `VisionReason` cerrado (`clear`/`dim-light`/`darkness`/`blocked-visual-path`/`darkvision-out-of-range`), `VisionTrace` con fuente `board-light`/`visual-path`/`intrinsic-perception` (deliberadamente sin `effectInstanceId` — Vision no se origina en un `EffectInstance`).
  - **Composición con `ConcealmentAssessment`** (`composeConcealmentAssessment`): severidad máxima nunca aditiva (`total > partial > none`), miss chance del severity ganador (nunca se suman 20%+50%), `directTargetingAllowed`/`requiresTargetSquare`/`opportunityAttackAllowed` derivan del mismo flag compuesto; `traces` (efectos) y `visionTraces` (Vision) se conservan por separado, sin sintetizar `EffectInstance` falsos.
  - **`resolve-attack` extendido** con `target: {kind:"combatant";combatantId}|{kind:"square";position}` (unión discriminada, un solo comando — no comandos paralelos). Objetivo directo se rechaza server-side si `directTargetingAllowed===false` sin revelar la posición real; targeting por casilla resuelve internamente si hay un atacable (cualquier celda de una huella multiposición es válida), consume intento/acción/munición igual en casilla vacía u ocupada, y nunca revela si la casilla estaba vacía (mismo mensaje de fallo genérico, sin RNG extra para el caso vacío).
  - **Hallazgo arquitectónico durante la implementación**: Cobertura Total (Line of Effect) debe evaluarse antes que Ocultación Total (Vision) — ambos assessments comparten hoy la misma fuente de bloqueo provisional, así que cualquier escenario de LoE bloqueado también aparece como Vision bloqueado; el gate de targeting directo por Vision se ubica deliberadamente después del gate de Line of Effect ya existente para preservar esa precedencia.
  - Consecuencia correcta y esperada: `directTargetingAllowed`/`requiresTargetSquare` existían en `ConcealmentAssessment` desde Sprint 046 pero ningún handler los hacía cumplir — Sprint 053B es el primer código que los aplica, lo cual también alcanza a los atacantes Cegados (su propia Ocultación Total, ya declarativa desde Sprint 047, ahora también los obliga a atacar por casilla).
  - Alcance parcial declarado: solo consecuencias ofensivas de oscuridad (Concealment, targeting, miss chance, Darkvision) — **no** incluye pérdida de Destreza a la CA por oscuridad, -2 CA, velocidad mitad, penalizadores de habilidades, ni aplicación automática de `srd_blinded` por oscuridad (la oscuridad es contextual por observador, Blinded es una condición persistente del combatiente).
  - Tests: `tests/vision-core.test.mjs` (17, unitarios de Vision + composición), `tests/blind-targeting-server.test.mjs` (15, targeting directo/por casilla + regresiones de Cover/LoE/Blinded).
  - **Validación real ejecutada (2026-07-24)**: `npm test` **542/542**, `npm run typecheck` 0 errores (3 workspaces), `npm run build` los 3 en verde, `node scripts/e2e-websocket.mjs` **100/100** aserciones exit 0, `npm run test:ui` **7/7** escenarios Playwright.
  - Registry: nueva Rule ID `DEFENSE-VISION` (Parcial) en `docs/rules/registry.md`; `EFFECT-BLINDED` y `DEFENSE-LINE-OF-EFFECT` actualizadas para reflejar que Vision ya no está pendiente.

### HISTÓRICO: Sprint 050.1 — Panel de Estados del GM
...
- Política efectiva de AdO bajo Ocultación Total (fuera de alcance explícito de Sprint 053B). Vision (luz/Darkvision) y targeting por casilla para ocultación total ya están implementados como fuente productiva de `DEFENSE-CONCEALMENT` — ver `DEFENSE-VISION`.
- Consecuencias defensivas/de movimiento de oscuridad (pérdida de Destreza a la CA, -2 CA, velocidad mitad, penalizadores de habilidades), Low-Light Vision, Blindsight/Blindsense/Tremorsense, niebla/humo, invisibilidad — alcance explícitamente diferido de `DEFENSE-VISION` (gap G-03 de `docs/audits/combat-rules-deviations.md`, ahora parcialmente cerrado).
