...
### FASE ACTUAL: Sprint 041 (MOVE-RUN) completado (validación Windows pendiente); Sprint 040 (Document Architecture Cleanup) listo para cierre arquitectónico; Sprint MOVE-WITHDRAW completado (validación Windows pendiente); Sprint 038 en gate; Sprint 039 congelado
  ...
  **Sprint 041 — Correr (Run)**
  ...
  - Tests: `run.test.mjs` 21/21 puros en verde real (`node --test` contra `dist/`) + `run-server.test.mjs` (18 casos servidor, escritos, vía `tsx` en Windows). Typecheck estricto (shared+web+server) en verde con todo el código integrado. `npm test` completo intentado: 20/70 pasan (esbuild `TransformError` en el resto, causa ambiental, no de código). `build:shared`/`build:server` verdes (`tsc` puro); `build:web` bloqueado (Rollup linux-x64 ausente). `node scripts/e2e-websocket.mjs`: se intentó arrancar el servidor compilado (`dist/index.js`) directamente para evitar `tsx`, pero `@dnd-tactical/shared` resuelve a `src/index.ts` en runtime igual — mismo bloqueo de raíz. `npm run test:ui` (Playwright): bloqueado (mismo Rollup). **Todo pendiente de Windows, DoD NO declarado completo** — ver gate de cierre en `walkthrough.md` y `docs/designs/run-design.md` §8. Regresión verificada dos veces (implementación y gate de cierre, ambas vía `git stash`): los mismos 11 fallos preexistentes en la suite pura aparecen idénticos antes y después de este sprint (ajenos a MOVE-RUN).
