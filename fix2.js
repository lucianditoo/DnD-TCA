...
const regex = /export function createRuleEvaluator.*?export const Rules = createRuleEvaluator\(effectsCatalog\);/s;
...
if (regex.test(code)) {
    ...
    // Now just need to ensure getEffectiveAbilityModifier and getSpecialManeuverSizeModifier are somewhere in the file.
    // Wait, are they in the file currently?
    if (!code.includes('export function getEffectiveAbilityModifier')) {
        code = code.replace('export const Rules = createRuleEvaluator(effectsCatalog);', `export const Rules = createRuleEvaluator(effectsCatalog);
...
export function getSpecialManeuverSizeModifier(sizeCategory: SizeCategory): number {
  return getSizeRule(sizeCategory).grappleModifier;
    ...
    // also clean up any floating pieces of code matching `): boolean {` without a function header
    // Actually regex replacing the block might have left some remnants. We can leave it for manual inspection if typecheck fails.
